...
export default function TopBar({ user }: { user: CurrentUser }) {
  ...
  return (
    <header
      ...
      style={{ background: "linear-gradient(90deg, rgba(124,147,175,0.16), var(--paper) 75%)" }}
    >
      ...
      <nav className="ml-auto flex items-center gap-4 text-sm">
        <Link href="/dashboard" className="font-semibold hover:text-[var(--denim)]">
          Dashboard
        </Link>
        {user.isHrAdmin && (
          <Link href="/admin" className="font-semibold hover:text-[var(--denim)]">
            Admin
          </Link>
        )}
        <Link href="/account" className="font-semibold hover:text-[var(--denim)]">
          {user.name}
        </Link>
        <button onClick={logout} className="rounded-lg border border-black/15 bg-white px-3 py-1.5 text-xs font-semibold hover:border-[var(--denim)]">
          Sign out
        </button>
      </nav>
    </header>
