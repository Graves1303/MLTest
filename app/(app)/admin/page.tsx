      ...
      <section className="bg-white border border-black/10 rounded-xl p-5 mb-6">
        ...
        <form onSubmit={createUser} className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
          ...
          <label className="block">
            ...
            <input value={form.level} onChange={(e) => setForm({ ...form, level: e.target.value })} className="w-full border border-black/18 rounded-lg px-3 py-2 text-sm" />
          </label>
          <label className="block sm:col-span-2">
            <span className="block text-xs font-semibold text-[var(--ink-soft)] mb-1.5">Level context (optional)</span>
            <input value={form.levelContext} onChange={(e) => setForm({ ...form, levelContext: e.target.value })} placeholder="e.g. Performance measured on individual outputs & completions of projects under direction" className="w-full border border-black/18 rounded-lg px-3 py-2 text-sm" />
          </label>
          <label className="block">
